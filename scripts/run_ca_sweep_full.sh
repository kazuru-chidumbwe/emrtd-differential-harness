...
for p in "${profiles[@]}"; do
  go run ./cmd/tc-ca-01 -profile "$p" -log-dir "$LOG_DIR" -variant baseline -suite-id ca-01-sweep -run-index "$idx" >/dev/null
  g_base=$((g_base+1)); idx=$((idx+1))
  go run ./cmd/tc-ca-01-mitigated -profile "$p" -log-dir "$LOG_DIR" -variant mitigated -suite-id ca-01-sweep -run-index "$idx" >/dev/null
  g_mit=$((g_mit+1)); idx=$((idx+1))
  java -cp "$JAR" org.emrtd.harness.jmrtd.TcCa01Runner -profile "$p" -log-dir "$LOG_DIR" -variant baseline -suite-id ca-01-sweep -run-index "$idx" >/dev/null
  j_base=$((j_base+1)); idx=$((idx+1))
  java -cp "$JAR" org.emrtd.harness.jmrtd.TcCa01MitigatedRunner -profile "$p" -log-dir "$LOG_DIR" -variant mitigated -suite-id ca-01-sweep -run-index "$idx" >/dev/null
