...
**Paper / package cite (bpfix-style):** tag **`v1.0.4`** · permanent link  
https://github.com/kazuru-chidumbwe/emrtd-differential-harness/tree/v1.0.4

Prefer citing the **tag name**. Short hashes below are illustrative; peel with `git rev-parse <tag>^{}`.

| Tag | Purpose |
| --- | --- |
| [`v1.0.4`](https://github.com/kazuru-chidumbwe/emrtd-differential-harness/releases/tag/v1.0.4) | **Live paper cite pin** — CITATION.cff / SECURITY.md / frozen schemas enforced; CI on `main`; same path-sanitized digests as `v1.0.3`. |
| [`v1.0.3`](https://github.com/kazuru-chidumbwe/emrtd-differential-harness/releases/tag/v1.0.3) | Path-sanitized locked-run digests. Superseded for *cite* by `v1.0.4`. |
...
| [`locked-runs-2026-08-10`](https://github.com/kazuru-chidumbwe/emrtd-differential-harness/releases/tag/locked-runs-2026-08-10) | **Live** path-sanitized locked-run zip (also attached on SemVer Releases) |
...
- **Paper / package citation** → **`v1.0.4`** (permanent tree URL above). See [`CHANGELOG.md`](../CHANGELOG.md).
...
**Digest vs tag:** Suite digests (`6c6cbbd1…`, `2efd190e…`, CA `43f9bd1d…`) are hashes of locked evidence trees. `v1.0.4` pins harness *source semantics* that match those claims. Regenerating suites reproduces Observability Scores but not identical timestamped hashes.
